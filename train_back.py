...
parser.add_argument("--decay_epochs", type=int, default=80,
                    help="epoch to start linearly decaying the learning rate to 0. (default:80)")
...
for epoch in range(0, args.epochs):
    ...
    for i, data in progress_bar:
        ...
        # Update D_B weights
        optimizer_D_B.step()
        Loss_D = (loss_identity_A + loss_identity_B).item()
        Loss_G_Total = errG.item()
        ...
        progress_bar.set_description(
            ...
            f"Loss_D: {Loss_D:.4f} "
            f"Loss_G_Total: {Loss_G_Total:.4f} "
            f"Loss_G_identity: {Loss_G_Identity:.4f} "
            f"Loss_G_GAN: {Loss_G_GAN:.4f} "
            f"Loss_G_cycle: {Loss_G_Cycle:.4f}")
            
        Running_Loss_D += Loss_D
        Running_Loss_G_Total += Loss_G_Total
        Running_Loss_G_Identity += Loss_G_Identity
        Running_Loss_G_GAN += Loss_G_GAN
        Running_Loss_G_Cycle += Loss_G_Cycle
    ...
    writer.add_scalar("Running_Loss_D: ", Running_Loss_D / len(dataloader), epoch)
    writer.add_scalar("Running_Loss_G_Total: ", Running_Loss_G_Total / len(dataloader), epoch)
    writer.add_scalar("Running_Loss_G_Identity: ", Running_Loss_G_Identity / len(dataloader), epoch)
    writer.add_scalar("Running_Loss_G_GAN: ", Running_Loss_G_GAN / len(dataloader), epoch)
    writer.add_scalar("Running_Loss_G_Cycle: ", Running_Loss_G_Cycle / len(dataloader), epoch)
